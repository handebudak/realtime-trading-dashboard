...
import { TrendingUp, TrendingDown, Wifi, WifiOff, ZoomIn, ZoomOut, Maximize2 } from 'lucide-react'
...
export default function DayTradingChart({ symbol: propSymbol }: DayTradingChartProps) {
  ...
  const { isConnected } = useWebSocket({
    ...
    enabled: !loading, // Only enable after initial data loaded
    onMessage: (message) => {
      if (message.type === 'kline' && chartData) {
        // Update on every tick (real-time), not just when closed
  ...
  const updateChartWithRealTimeData = (newKline: any) => {
    if (!chartData) return

    setChartData((prevData) => {
      if (!prevData) return null

      const updatedKlines = [...prevData.klines]
      ...
      if (lastKline && lastKline.openTime === newKline.openTime) {
        ...
        updatedKlines[updatedKlines.length - 1] = {
      ...
      } else if (newKline.isClosed) {
        // Add new candle only when previous one is closed
        updatedKlines.push({
          openTime: newKline.openTime,
          open: newKline.open,
          high: newKline.high,
        ...
        if (updatedKlines.length > 100) {
          updatedKlines.shift()
        }
      }

      // Update price change
      const firstPrice = parseFloat(updatedKlines[0].close)
      const lastPrice = parseFloat(newKline.close)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3002/api/v1/charts/day-trading/${selectedSymbol}`)
        ...
        if (result.success) {
          ...
          if (klines.length > 1) {
            ...
            setCurrentPrice(lastPrice)
            // Calculate price change with debug logging
            const priceChangeValue = ((lastPrice - firstPrice) / firstPrice) * 100
            setPriceChange(priceChangeValue)
            
            // Debug logging
            console.log('Day Trading Chart Debug:', {
              firstPrice,
              lastPrice,
              priceChangeValue,
        ...
        console.error('Day trading chart error:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  // Chart config created ONCE - only depends on settings, not data updates! (MUST be before early returns!)
  ...
  }}, [chartData?.symbol, chartData?.interval]) // Only recreate on symbol/interval change, NOT data updates!
  ...
  return (
    <div ref={containerRef} className="glass rounded-2xl shadow-lg border border-[var(--card-border)] bg-[var(--card-bg)]">
      {/* Header */}
      <div className="px-4 py-3 border-b border-[var(--card-border)]">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--muted)]">15-minute candles for intraday trades, last 4 hours</p>
            {currentPrice > 0 && (
                ...
                <div className="flex items-center">
                  {priceChange >= 0 ? (
                    <>
                      ...
                      <span className="text-sm font-bold text-[#2EBD85]">+{priceChange.toFixed(4)}%</span>
                    </>
                  ) : (
                    <>
                      <TrendingDown className="w-4 h-4 text-[#F6465D] mr-1" />
                      <span className="text-sm font-bold text-[#F6465D]">{priceChange.toFixed(4)}%</span>
                    </>
                  )}
                </div>
              </div>
