...
interface PositionTradingChartProps {
  symbol?: string
}
...
export default function PositionTradingChart({ symbol: propSymbol }: PositionTradingChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3002/api/v1/charts/position-trading/${selectedSymbol}`)
        ...
        console.error('Position trading chart error:', err)
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className="px-4 py-3 border-b border-[var(--card-border)]">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--muted)]">Daily candles for long-term investment, last 1 month</p>
            {currentPrice > 0 && (
