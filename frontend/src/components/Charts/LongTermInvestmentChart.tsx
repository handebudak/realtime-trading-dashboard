...
interface LongTermInvestmentChartProps {
  symbol?: string
}
...
export default function LongTermInvestmentChart({ symbol: propSymbol }: LongTermInvestmentChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3002/api/v1/charts/long-term-investment/${selectedSymbol}`)
        ...
        console.error('Long-term investment chart error:', err)
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className="px-4 py-3 border-b border-[var(--card-border)]">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--muted)]">Daily candles for annual investment strategy, last 1 year</p>
            {currentPrice > 0 && (
