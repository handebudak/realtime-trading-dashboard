...
export default function ScalpingChart({ symbol: propSymbol }: ScalpingChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3002/api/v1/charts/scalping/${selectedSymbol}`)
        ...
        console.error('Scalping chart error:', err)
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className="px-4 py-3 border-b border-[var(--card-border)]">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--muted)]">5-minute candles for quick profit taking, last 1 hour</p>
            {currentPrice > 0 && (
